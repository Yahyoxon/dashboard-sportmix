...
const AddProduct = (props) => {
        ...
        if (e.target.files.length > 0) {
            for (let i = 0; i < e.target.files.length; i++) {
                if (e.target.files[i].size < 11256000) {
                    ...
                    setImagesPreview((prevState) => [...prevState, URL.createObjectURL(e.target.files[i])]);
                }
                else {
    ...
    const addProduct = async (e) => {
        ...
        images.map((image) => {
            ...
            uploadTask.on(
                ...
                async () => {
                    const img = await storage.ref("productsImage").child(image.name).getDownloadURL()
                    // .then((urls) => {
        ...
        Promise.all(promises)
            ...
            .then(() => {
                const response = axios.post(`https://api.sport-mix.uz/api/products/create`,
                    {
                        "name": name,
    ...
    //     try {
    //         if (urls.length === countImage) {
    //             const response = await axios.post(`https://api.sport-mix.uz/api/products/create`,
    //                 {
    //                     "name": name,
    //                     "description": description,
    //                     "price": price,
                ...
                <div className="card__body">
                    <form onSubmit={addProduct}>
                        ...
                        <div className="images-labels">
                            ...
                            <input style={{ display: "none" }} type="file" id="img" multiple required onChange={handleChange} /><br />
                        </div>
                        <div className="topnav__input">
                            {isUpload === false ?
                                <button style={{ width: "100%", padding: "12px" }} type="submit" className="badge badge-primary">Добавить</button>
                                :
                                <div className="prg">
...
//     const handleChange = (e) => {
//         setcountImage(e.target.files.length)
//         if (e.target.files.length > 0) {
//             for (let i = 0; i < e.target.files.length; i++) {
//                 if (e.target.files[i].size < 11256000) {
//                     setImages((prevState) => [...prevState, e.target.files[i]]);
...
//                     }
//                 )
//                 if (response.data.message !== "Product is not created.") {
//                     window.location = "/products"
//                 }
