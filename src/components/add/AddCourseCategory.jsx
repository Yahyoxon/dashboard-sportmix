...
const AddCourseCategory = () => {
    ...
    const publishCategory = async (e) => {
        ...
        if (isImageUploaded === true) {
            ...
            const uploadTask = storage.ref(`courseCategoryImage/${categoryNewImage.name}`).put(categoryNewImage);
            uploadTask.on(
                "state_changed",
                snapshot => {
                ...
                async () => {
                    const u = await storage.ref("courseCategoryImage").child(categoryNewImage.name).getDownloadURL()

                    const response = await axios.post(`https://api.sport-mix.uz/api/course/categories/create`,
                        {
                            "name": categoryName,
                            "poster": u
                        }
